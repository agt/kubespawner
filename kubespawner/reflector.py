...
class NamespacedResourceReflector(LoggingConfigurable):
    ...
    def _create_resource_key(self,resource):
        """
	Maps a Kubernetes resource object onto a hashable Dict key; subclass may
	override if `resource.metadata.name` is insufficiently unique (e.g. pods across multiple namespaces)
        """
    ...
    def _list_and_update(self):
        ...
        if self.list_method_omit_namespace is not True:
